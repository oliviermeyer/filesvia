$axure.internal(function($ax) {
        ...
        if(!viewId) {
            $axure('*').each(function(diagramObject, elementId) {
                ...
                if(diagramObject.type == 'horizontalLine' || diagramObject.type == 'verticalLine') {
                    ...
                    if(disabledImage && $ax.style.IsWidgetDisabled(elementId)) {
                        $ax.style.applyImage(elementId, disabledImage, 'disabled');
                        return;
                    }
                    var selectedImage = $ax.style.getElementImageOverride(elementId, 'selected') || images['selected~'];
                    if(selectedImage && $ax.style.IsWidgetSelected(elementId)) {
                        $ax.style.applyImage(elementId, selectedImage, 'selected');
                        return;
                    }
    ...
    var _applyAdaptiveViewOnObject = function(diagramObject, elementId, viewIdChain, viewId, limboIds, hiddenIds) {
        ...
        var state = ($ax.style.IsWidgetDisabled(elementId) && "disabled")
            || ($ax.style.IsWidgetSelected(elementId) && "selected") || "normal";
    ...
    var _matchImage = function(id, images, viewIdChain, state) {
        ...
        // Now look for normal override
        override = $ax.style.getElementImageOverride(id, 'normal');
        if(override) return override;

        // then check all the images for the normal if we can't find that state
        for(var i = viewIdChain.length - 1; i >= 0; i--) {
            var viewId = viewIdChain[i];
            var img = images["normal~" + viewId];
            if(img) return img;
        }
    ...
    var _getImageForStateAndView = $ax.adaptive.getImageForStateAndView = function(id, state) {
