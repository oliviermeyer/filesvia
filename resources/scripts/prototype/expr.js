...
$axure.internal(function($ax) {
    ...
    _exprFunctions.SetWidgetRichText = function(ids, value, plain) {
        ...
        for(var i = 0; i < ids.length; i++) {
            ...
            if(plain) {
                ...
                var span = $('<span></span>');
                if(spans.length > 0) span.attr('style', $(spans[0]).attr('style'));
                span.html(value);
                var p = $('<p></p>');
                var ps = $jobj(id).find('p');
                if(ps.length > 0) p.attr('style', $(ps[0]).attr('style'));
                p.append(span);
    ...
    _exprFunctions.GetWidgetText = function(ids) {
        return $ax('#' + $ax.repeater.applySuffixToElementId(ids[0], '_input')).text();
    };
