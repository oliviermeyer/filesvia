...
$axure.internal(function($ax) {
    ...
    for(var i = 0; i < EVENT_NAMES.length; i++) {
        ...
        _event[eventName] = (function(event) {
            return function(elementId, fn) {
                ...
                //see notes below
                else if($ax.IsTreeNodeObject(type)) _attachTreeNodeEvent(elementId, event, fn);
                else if($ax.IsButtonShape(type) && (event == 'focus' || event == 'blur')) {
                    _attachDefaultObjectEvent($jobj($ax.repeater.applySuffixToElementId(elementId, '_img')), elementId, event, fn);
                } else {
                    var inputId = $ax.INPUT(elementId);
                    var isInput = $jobj(inputId).length != 0;
                    var id = isInput && (event == 'focus' || event == 'blur') ? inputId : elementId;
                    _attachDefaultObjectEvent($jobj(id), elementId, event, fn);
                }
            };
        })(eventName);
    }
    ...
    var _initializeObjectEvents = function(query) {
        ...
        query.filter(function(obj) {
            ...
            $jobj($ax.INPUT(elementId)).bind('focus', function() {
                var id = this.id;
                var inputIndex = id.indexOf('_input');
                if(inputIndex == -1) return;
                id = id.substring(0, inputIndex);

                if(!$ax.placeholderManager.isActive(id)) return;
                $ax.placeholderManager.updatePlaceholder(id, false, true);
            }).bind('blur', function() {
                var id = this.id;
                var inputIndex = id.indexOf('_input');
                if(inputIndex == -1) return;
        ...
        query.filter(function(obj) {
            ...
            if(jobj.hasClass('sub_menu')) {
                var tableCellScriptId = $ax.getElementIdFromPath([obj.parentCellId], { relativeTo: $ax.repeater.getScriptIdFromElementId(elementId) });
